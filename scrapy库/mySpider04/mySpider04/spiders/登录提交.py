...
class Example3Spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in cookie.split(';'):
            cookie_sict[i.split('=')[0]] = i.split('=')[1]
        yield scrapy.Request(url=self.start_urls[0],callback=self.parse,cookies=cookie_sict)
    def parse(self, response):
        #查找出来的参数
        print(response.request.url)
        print(response)
        print(re.findall('zbj15963578', response.text, re.S))
